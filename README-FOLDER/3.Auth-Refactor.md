...
#### B. Create a new route endpoint middleware. （getMe）
...
#### C. Import the route endpoint middleware and protect middleware in route.
...
- 写middleware必须注意两点，第一点是可能需要改变req的内容，第二点是必须有`next()`.
- 在这个设计流程中，经过`protect`中间件的route都会在`req`中取得user全部的信息，而protect中间件的主要作用是在解析token，然后寻找对应的user，如果有user就`增加`req的内容，如果没有就报错。
...
```js
router.get('/someRoute', protect, authorize('user', 'admin'), endpointFunc);
```
...
6. `./apis/auth.js`(*3.5) -- import 1 new route endpoint middleware and 2 new middlewares
